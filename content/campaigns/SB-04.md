...
| Enterprise | T1552.005 | Unsecured Credentials: Cloud Instance Metadata API | 캠페인 당시 공격 그룹은 장악한 K8s 워커 노드에서 AWS 인스턴스 메타데이터 서비스(IMDS)에 접근하여 노드에 할당된 IAM Role(Instance Profile) 자격 증명을 탈취했다. |[[AL-001]]|
| Enterprise | T1213.006 | Data from Information Repositories: Databases | 캠페인 당시 공격 그룹은 탈취한 IAM Role 권한을 이용해 내부망의 RDS 데이터베이스에 접근하고, 타겟 고객 데이터(JSON)를 대량으로 조회 및 유출했다. |[[AL-002]]|
