...
Security 4688은 process creation 원본 필드로 먼저 검색한다. command line auditing이 꺼져 있으면 `winlog.event_data.CommandLine`이 비어 있을 수 있으므로 `process.executable`, `NewProcessName`, `message`를 함께 본다.
